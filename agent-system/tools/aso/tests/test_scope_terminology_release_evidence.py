...
class ScopeTerminologyReleaseEvidenceTests(unittest.TestCase):
    ...
    def test_runtime_schema_3_1_1_maps_to_historical_3_1_0_contract_file(self) -> None:
        ...
        self.assertFalse((RUNTIME_SCHEMA_DIR / "runtime_state_3_1_1.contract.json").exists())
        self.assertIn("Runtime Schema `3.1.1` continues to use the packaged contract file", authority_map)
        self.assertIn("agent-system/09_validators/schemas/runtime_state_3_1_0.contract.json", authority_map)
        self.assertIn("no duplicate `3_1_1` contract file is authoritative", (REPO_ROOT / "README.md").read_text(encoding="utf-8"))
    ...
    def test_current_release_context_is_p58f1_no_upgrade_3_7_9(self) -> None:
        root_readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
        install_readme = (REPO_ROOT / "README_INSTALL.md").read_text(encoding="utf-8")

        self.assertIn("P58/P58F1 no-upgrade correction line", root_readme)
        self.assertIn("P58/P58F1 no-upgrade correction context", install_readme)
        self.assertIn("governed `3.7.9` package/governance tuple", root_readme)
        self.assertIn("package version `3.7.9`", install_readme)
