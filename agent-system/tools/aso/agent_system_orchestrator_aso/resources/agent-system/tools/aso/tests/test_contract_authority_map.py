...
CONTRACT_3_2_0 = REPO_ROOT / "agent-system" / "09_validators" / "schemas" / "runtime_state_3_2_0.contract.json"
...
class ContractAuthorityMapTests(unittest.TestCase):
    def test_p5_authority_map_documents_active_chain(self) -> None:
        ...
        self.assertIn("| S1.140 |", authority_map)
        self.assertIn("package_version: 3.8.0", authority_map)
        self.assertIn("runtime_schema_version: 3.2.0", authority_map)

    def test_runtime_schema_3_2_0_uses_mapped_base_contract_file(self) -> None:
        authority_map = AUTHORITY_MAP.read_text(encoding="utf-8")

        self.assertTrue(CONTRACT_3_1_0.is_file())
        self.assertFalse(CONTRACT_3_2_0.exists())
        self.assertIn("runtime_state_3_1_0.contract.json", authority_map)
        self.assertIn("Runtime Schema `3.2.0`", authority_map)
        self.assertIn("Do not add a second `runtime_state_3_2_0.contract.json`", authority_map)
