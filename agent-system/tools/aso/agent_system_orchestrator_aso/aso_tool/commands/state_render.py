...
LEGACY_COMPATIBILITY_VIEW_TYPES = (
    "GAP_REGISTER",
    "AGENT_RESULTS_LOG",
...
def materialized_view_relpaths() -> tuple[str, ...]:
    return tuple(
        f"project-runtime/{view_type}.md"
        for view_type in (*MATERIALIZED_VIEW_TYPES, *LEGACY_COMPATIBILITY_VIEW_TYPES)
    )
...
def _legacy_compatibility_value(value: object) -> str:
    if isinstance(value, list):
        if not value:
            return "NONE"
        return json.dumps(value, sort_keys=True)
...
def render_legacy_compatibility_view(view_type: str, sidecars: dict[str, dict[str, object]]) -> str:
    project_state = _content(sidecars.get("PROJECT_STATE", {}))
    task_registry = _content(sidecars.get("TASK_REGISTRY", {}))
    ...
    active_gap_count = len(active_gaps) if isinstance(active_gaps, list) else 0

    sections: dict[str, list[str]] = {
        "GAP_REGISTER": [
            f"ACTIVE_GAP_COUNT: {active_gap_count}",
            f"ACTIVE_GAPS: {_legacy_compatibility_value(active_gaps)}",
            f"PROJECT_STATUS: {_legacy_compatibility_value(project_state.get('project_status'))}",
        ],
        "AGENT_RESULTS_LOG": [
        ...
        "ORCHESTRATOR_EVENTS_LOG": [
            f"CURRENT_GATE: {_legacy_compatibility_value(current_gate.get('gate_id'))}",
            f"NEXT_ACTION: {_legacy_compatibility_value(next_action.get('action_type'))}",
            "EVENT_COUNT: 0",
        ],
        "STATUS_SUMMARY": [
            f"PROJECT_STATUS: {_legacy_compatibility_value(project_state.get('project_status'))}",
            f"CURRENT_PHASE: {_legacy_compatibility_value(project_state.get('current_phase'))}",
            f"NEXT_ACTION: {_legacy_compatibility_value(next_action.get('action_type'))}",
            f"TARGET_ROLE: {_legacy_compatibility_value(next_action.get('target_role'))}",
            f"TASK_ID: {_legacy_compatibility_value(next_action.get('task_id'))}",
            f"CURRENT_GATE: {_legacy_compatibility_value(current_gate.get('status'))}",
        ],
    }
    lines = [
        "DERIVED COMPATIBILITY VIEW.",
        "Source: project-runtime/state/*.json",
        "Do not edit this file directly.",
        "Canonical state is JSON sidecar and governed runtime evidence.",
        "",
        f"# {view_type}",
        "",
        *sections[view_type],
        "MATERIALIZATION_STATUS: derived",
    ]
    return "\n".join(lines).rstrip() + "\n"
...
def materialize_compatibility_views(root: Path) -> tuple[dict[str, object], int]:
    ...
    legacy_writes: list[dict[str, object]] = []
    ...
    if not findings:
        for view_type in LEGACY_COMPATIBILITY_VIEW_TYPES:
            relpath = f"project-runtime/{view_type}.md"
            path = root / relpath
            rendered = render_legacy_compatibility_view(view_type, sidecars)
            try:
                path.parent.mkdir(parents=True, exist_ok=True)
                before = path.read_text(encoding="utf-8") if path.exists() else None
                path.write_text(rendered, encoding="utf-8")
                ...
                findings.append(
                    {
                        "rule_id": "STATE_RENDER_MATERIALIZE_004",
                        "severity": "error",
                        "title": "Legacy compatibility view write failed",
                        "details": f"{relpath}: {exc}",
                        "path": relpath,
                        "recommendation": "Repair workspace permissions and rerun aso state render --confirm-write.",
                    }
    ...
    return {
        ...
        "mutations_performed": bool(writes or legacy_writes or cache_writes),
        "legacy_compatibility_writes": legacy_writes,
        "state_cache_writes": cache_writes,
        "writes": writes,
        "findings": findings,
        "summary": {
            "errors": len(findings),
            "legacy_views_written": len(legacy_writes),
            "state_cache_writes": len(cache_writes),
            "sidecar_views_written": len(writes),
            "views_written": len(writes) + len(legacy_writes),
            "views_changed": sum(1 for item in (*writes, *legacy_writes) if item.get("changed")),
        },
