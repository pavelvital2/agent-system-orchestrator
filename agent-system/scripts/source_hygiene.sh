...
PYTHON_CACHE_ARTIFACT_REGEX='(^|/)__pycache__/|\.py[co]$'
PYTHON_TOOLING_CACHE_COVERAGE_REGEX='(^|/)(\.pytest_cache|\.mypy_cache|\.ruff_cache|\.coverage|htmlcov)(/|$)|(^|/)coverage\.xml$'
SOURCE_ARTIFACT_REGEX="(${PYTHON_CACHE_ARTIFACT_REGEX})|(${PYTHON_TOOLING_CACHE_COVERAGE_REGEX})"
PYTHON_BUILD_ARTIFACT_REGEX='(^|/)(__pycache__|build|dist|\.eggs)(/|$)|(^|/)[^/]+\.(egg-info|dist-info)(/|$)|\.py[co]$|(^|/)(\.pytest_cache|\.mypy_cache|\.ruff_cache|\.coverage|htmlcov)(/|$)|(^|/)coverage\.xml$'
...
filesystem_python_build_artifacts() {
  find . \
    ...
    -o \( \
      -type d \( -name '__pycache__' -o -name 'build' -o -name 'dist' -o -name '.eggs' -o -name '*.egg-info' -o -name '*.dist-info' -o -name '.pytest_cache' -o -name '.mypy_cache' -o -name '.ruff_cache' -o -name '.coverage' -o -name 'htmlcov' \) \
      -o -type f \( -name '*.pyc' -o -name '*.pyo' -o -name '.coverage' -o -name 'coverage.xml' \) \
    \) -print | sed 's#^\./##'
}
...
fail_with_paths \
  ...
  "$(printf '%s\n' "$tracked_paths" | matching_paths "$SOURCE_ARTIFACT_REGEX")"
fail_with_paths \
  "staged Python/tooling cache and coverage artifacts are not publishable" \
  "$(printf '%s\n' "$staged_paths" | matching_paths "$SOURCE_ARTIFACT_REGEX")"
fail_with_paths \
...
fail_with_paths \
  "git archive HEAD contains Python/tooling cache and coverage artifacts" \
  "$(printf '%s\n' "$archive_paths" | matching_paths "$SOURCE_ARTIFACT_REGEX")"
fail_with_paths \
