...
assert_version_changelog_coherence() {
  awk -v pkg="CURRENT_PACKAGE_VERSION: 3.7.2" \
    -v governance="CURRENT_GOVERNANCE_RULESET_VERSION: 3.7.2" \
  ...
  ' "$PACKAGE_VERSIONING" || die "PACKAGE_VERSIONING active version constants missing 3.7.2 / 3.7.2 / 3.1.0"
  ...
  ' "$PACKAGE_README" || die "README current active tuple missing 3.7.2 / 3.7.2 / 3.1.0 Artifact Package Model P5 marker"
  ...
  printf 'PASS: version coherence asserts active 3.7.2 package/governance with runtime schema 3.1.0\n'
  PASS_COUNT=$((PASS_COUNT + 1))
}
