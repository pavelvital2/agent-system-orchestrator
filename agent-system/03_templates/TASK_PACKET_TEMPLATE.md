...
- RESULT_PATH must use the `AUDIT_RESULT_` prefix for new auditor result
  files;
- new worker result files must be under `project-runtime/results/worker/`;
- new auditor result files must be under `project-runtime/results/audit/`;
- `project-runtime/agent-results/` is compatibility/read-only legacy storage
  for historical worker RESULT evidence and must not be used for new result
  files;
