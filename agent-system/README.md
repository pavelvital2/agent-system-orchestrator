...
This P4.1 hotfix package records the active package metadata as the governed
`3.6.1` package/governance tuple with runtime schema `3.1.0`. It preserves the
Project Factory P1 command boundary and the P2/P3 Runtime Schema `3.1.0`
sidecar envelope while fixing workspace/package mode guard handling, derived
runtime Markdown materialization from JSON sidecars, and lifecycle termination
event recording after RESULT before audit routing.
...
state source. The active package version is `3.6.1` and the active runtime
...
under the current package/governance `3.6.1` tuple with Runtime Schema `3.1.0`
...
wizard. It remains available in package version `3.6.1`; existing P1/P0
...
RUNTIME_STATE_RELEASE_MARKER: workspace-bootstrap-runtime-lifecycle-p4.1
